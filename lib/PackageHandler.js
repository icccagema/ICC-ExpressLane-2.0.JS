    ...
    return {
        ...
        ProcessPackages: function(crmItemLines, itemCatalogDict, crmOrderInfo) {
            ...
            ValidItems.forEach(function(o, i, a) {
                ...
                //catalogInfo.SubType = ['null', null].includes(catalogInfo.SubType) ? '(Empty)' : catalogInfo.SubType;
                catalogInfo.SubType = ['null', null].includes(catalogInfo.SubType) ? '' : catalogInfo.SubType;
                console.log(`${catalogInfo.Type}/${catalogInfo.SubType}/${itemCode}`);
                fs.readdirSync(`I:/Express/Manufacturing/`, { withFileTypes: true }).filter(e => e.isDirectory()).filter(d => `${d.name}`)
                ...
                frameCol.PackagePath = packagePath.split(/\\|\//).slice(1).join('/');
