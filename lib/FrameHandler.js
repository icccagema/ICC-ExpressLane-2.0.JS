        ...
        ParseFrames: function(mfgText, prodCode) {
            ...
            var shiploose =
                mfgItems.
                    filter(mfgItem =>
                        (
                            [`{Loose:${prodCode}}`, ''].includes(mfgItem.Assembled.trim()) ||
                            frames[mfgItem.Assembled.trim()] == undefined
                        ) &&
                        !mfgItem.Description.toLowerCase().includes('assembled partition frame')).
                    map(mfgItem => {
                        mfgItem.Assembled = mfgItem.Assembled.includes('{') ? '' : mfgItem.Assembled ;
                        return mfgItem;
                    });
            ...
            prodFrameCol.ProductCode = prodCode;
